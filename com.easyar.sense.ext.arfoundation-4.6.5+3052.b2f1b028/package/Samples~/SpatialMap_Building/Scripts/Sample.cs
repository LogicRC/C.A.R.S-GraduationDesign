...
namespace Sample
{
    public class Sample : MonoBehaviour
    {
        ...
        public GameObject ARFoundationOrigin;
        public GameObject ARFoundationSession;
        ...
        private void Awake()
        {
            ...
            picker.FrameSource = ARComponentPicker.SingleSelection.Disable;
            ARFoundationOrigin.SetActive(false);
            ARFoundationSession.SetActive(false);
            var camera = new GameObject("Camera", typeof(Camera)).GetComponent<Camera>();
            camera.clearFlags = CameraClearFlags.SolidColor;
            camera.backgroundColor = Color.gray;
            Status.text = "This sample requires AR Foundation >= 5.\n(EasyAR plugin and extension is compatible with version 4.)";
            GUIPopup.EnqueueMessage(Status.text, 1000, false);
#endif
        ...
        void Start()
        {
            raycastManager = ARFoundationOrigin.GetComponent<ARRaycastManager>();
            planeManager = ARFoundationOrigin.GetComponent<ARPlaneManager>();
        ...
        void Update()
        {
            if (Session.State >= easyar.ARSession.SessionState.Ready)
            {
            ...
            {
                if (Session.State >= easyar.ARSession.SessionState.Ready && (FrameSource.IsCustomCamera(Session.Assembly.FrameSource) || trialCounter.Value == DateTime.MinValue))
                {
